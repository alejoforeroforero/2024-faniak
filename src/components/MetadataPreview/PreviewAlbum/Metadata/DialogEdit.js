...
export default function DialogEditMetadata({
  ...
  const handleSave = () => {
    ...
    const payload = {
      ...
      title_language: form.title_language.trim(),
      release_date: formTrimmers.date(form.release_date),
      year: form.year.trim(),
      upc_code: form.upc_code.trim(),
      record_labels: [{
        name: form.record_label.trim(),
        catalog_n: form.catalog_n.trim(),
      }],
      keywords: formTrimmers.list(form.keywords),
      description: form.description.trim(),
      special_thanks: form.special_thanks.trim(),
    }
